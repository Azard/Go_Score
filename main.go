...
func login(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		if aname != "" && tempPassword == apassword {
			...
			http.Redirect(w, r, "/course/se106/2014/grade/admin", http.StatusFound)
		...
		if id != 0 { //搜索到用户名
			if tempPassword == password { //登陆成功
				...
				http.Redirect(w, r, "/course/se106/2014/grade/student", http.StatusFound)
...
func logout(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, "/course/se106/2014/grade/", http.StatusFound)
	return
}
